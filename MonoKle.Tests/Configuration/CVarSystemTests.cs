...
namespace MonoKle.Configuration.Tests
{
    ...
    public class CVarSystemTests
    {
        ...
        [TestMethod]
        public void BindProperties_AutoGenerated_Smoke()
        {
            var toBind = new AutogenType
            {
                One = 1,
                Two = 2,
            };
            _cvarSystem.BindProperties(toBind, false, true);
            Assert.AreEqual(2, _cvarSystem.Identifiers.Count);
            Assert.AreEqual(1, toBind.One);
            Assert.AreEqual(2, toBind.Two);
            Assert.AreEqual(toBind.One, _cvarSystem.GetValue(nameof(toBind.One)));
            Assert.AreEqual(toBind.Two, _cvarSystem.GetValue(nameof(toBind.Two)));
            _cvarSystem.SetValue(nameof(toBind.One), 8);
            Assert.AreEqual(8, toBind.One);
            Assert.AreEqual(toBind.One, _cvarSystem.GetValue(nameof(toBind.One)));
        }

        [TestMethod]
        public void BindProperties_Prefix_Smoke()
        {
            var toBind = new PrefixType
            {
                One = 1,
                Two = 2,
            };
            ...
            Assert.AreEqual(1, toBind.One);
            Assert.AreEqual(2, toBind.Two);
            Assert.AreEqual(toBind.One, _cvarSystem.GetValue(PrefixType.PREFIX + nameof(toBind.One)), "Autogen identifier");
            Assert.AreEqual(toBind.Two, _cvarSystem.GetValue(PrefixType.PREFIX + "Three"), "Specific identifier");
            _cvarSystem.SetValue(PrefixType.PREFIX + nameof(toBind.One), 8);
            Assert.AreEqual(8, toBind.One);
            Assert.AreEqual(toBind.One, _cvarSystem.GetValue(PrefixType.PREFIX + nameof(toBind.One)));
        }
